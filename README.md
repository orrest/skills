...
|------|-------|----------------|
| Routing | `route` | The user-facing facade that knows the local skill set, selects the right worker or graph, delegates work to sub-agents, and returns after each delegated step finishes. |
...
The architecture keeps `route` as the facade, makes evidence collection explicit, and treats action skills as focused executors.
...
Prefer `route` as the entry point when the route is not already obvious. It can:
